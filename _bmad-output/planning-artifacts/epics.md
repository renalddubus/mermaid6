...
**Validation MVP.** Sur Chrome et Safari, vérifier le parcours exemple → édition → couleurs → PNG/SVG, un import invalide et le brouillon local. Automatiser les contrôles de rendu/export par type ; compléter par une vérification visuelle. Tester ensuite l'image publiée avec le démarrage documenté. La validation Firefox est reportée et ne bloque pas cette première livraison, sur décision du porteur du projet.
...
**Prochaine étape.** Relier le dépôt à GitHub et valider la release initiale. Safari est validé par le porteur du projet ; Firefox sera vérifié ultérieurement. Les comptes et volumes premium restent hors MVP.
